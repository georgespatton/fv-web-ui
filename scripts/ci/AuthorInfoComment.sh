...
do
    if [[ $f =~ FW-[0-9]{1,5} ]]; then
        ...
        if [[ $FOUND = true ]]; then
            ...
            ISSUE=${BASH_REMATCH}
            if [[ ! $(echo $issue_info | grep -o 'comments:[[:space:]].*') =~ The[[:space:]]pull[[:space:]]request[[:space:]]for[[:space:]]this[[:space:]]issue[[:space:]]was[[:space:]]made[[:space:]]by[[:space:]]https:\/\/github\.com\/${REVIEWER} ]]; then
                echo ${ISSUE} ": Adding pull request author info to issue:" ${AUTHOR}
                jira comment --noedit --comment="The pull request for this issue was made by https://github.com/$AUTHOR" ${ISSUE}
            else
                echo ${ISSUE} ": Issue already reviewed by" ${AUTHOR}
            fi
        else
